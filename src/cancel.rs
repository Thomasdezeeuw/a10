...
use crate::{sys, AsyncFd, SubmissionQueue};
...
pub trait Cancel {
    ...
    /// `MultishotAccept` will return more accepted connections after it's
    /// canceled. Simply keep accepting the connections and it will return
    /// `None` after all pending connections have been accepted.
    ///
