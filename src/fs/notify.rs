...
const NAME_MAX: usize = 255; // TODO: move to libc (from <Linux/include/uapi/linux/limits.h>).
const BUF_SIZE: usize = size_of::<libc::inotify_event>() + NAME_MAX + 1;

impl Watcher {
    /// Create a new file system watcher.
    pub fn new(sq: SubmissionQueue) -> io::Result<Watcher> {
        Ok(Watcher {
            // SAFETY: we're creating the fd, so it's valid.
            fd: unsafe { AsyncFd::from_raw_fd(syscall!(inotify_init1(libc::IN_CLOEXEC))?, sq) },
            buf: Some(Vec::new()),
    ...
    pub fn poll_next(
        ...
        loop {
            if let Some(mut buf) = self.buf.take() {
                buf.clear();
                buf.reserve(BUF_SIZE);
                self.read = self.read.fd().read(buf);
            }

            match Pin::new(&mut self.read).poll(ctx) {
                Poll::Ready(Ok(buf)) => {
                    // SAFETY: the kernel writes `inotify_event` to `buf`.
                    let buf_len = buf.len();
                    assert!(buf_len >= size_of::<libc::inotify_event>());
                    let buf = &*self.buf.insert(buf);
                    let event_ptr = buf.as_ptr().cast::<libc::inotify_event>();

                    // The path can contain null bytes as padding for alignment.
                    let len = unsafe { (&*event_ptr).len as usize };
                    assert!(buf_len >= size_of::<libc::inotify_event>() + len);
                    let path = unsafe {
                        slice::from_raw_parts(
                            event_ptr.cast::<u8>().add(size_of::<libc::inotify_event>()),
                            len,
                        )
                    };
                    let path_len = path
                    ...
                    let event: &'w Event = unsafe {
                        &*std::mem::transmute::<*const [u8], *const Event>(
                            ptr::slice_from_raw_parts(event_ptr.cast(), path_len),
                        )
                    };
                    ...
                    if mask & libc::IN_IGNORED != 0 {
                        _ = self.watching.remove(&event.event.wd);
